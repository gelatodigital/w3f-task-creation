...
contract Oracle1BalanceTaskCreator is AutomateTaskCreator {
    address public owner;
    uint256 public number;
    bytes32 public taskId;

    modifier onlyOwner() {
        require(msg.sender == owner, "Oracle1BalanceTaskCreator.onlyOwner");
        _;
    }
    ...
    function cancelTask() external onlyOwner {
        require(
            taskId != 0,
            "Oracle1BalanceTaskCreator.cancelTask: task not running"
        );
        _cancelTask(taskId);
        taskId = 0;
    }
    ...
    function createTask(string memory cid) external onlyOwner {
        require(
            taskId == 0,
            "Oracle1BalanceTaskCreator.cancelTask: task already running"
        );
        taskId = _createOracleTask(cid);
    }
