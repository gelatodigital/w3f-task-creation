...
contract OracleTaskCreator is AutomateTaskCreator {
    address public owner;
    uint256 public number;
    bytes32 public taskId;

    modifier onlyOwner() {
        require(msg.sender == owner, "OracleTaskCreator.onlyOwner");
        _;
    }
    ...
    function cancelTask() external onlyOwner {
        require(taskId != 0, "OracleTaskCreator.cancelTask: task not running");
        _cancelTask(taskId);
        taskId = 0;
    }
    ...
    function createTask(string memory cid) external onlyOwner {
        require(
            taskId == 0,
            "OracleTaskCreator.cancelTask: task already running"
        );
        taskId = _createOracleTask(cid);
    }
    ...
    function withdraw(address payable to, uint256 amount) external onlyOwner {
        ...
        require(sent, "OracleTaskCreator.withdraw: failed to withdraw");
    }
